...
def verify_syslog(device):
    try:
        response = device.runCmds(['show logging last 7 days threshold errors'], 'text')
        if len(response[0]['response']['output']) == 0:
            return True
        else:
            return False
    ...
    try:
        ...
        for line in response[0]['response']['output'].split('\n')[1:]:
            if 'loop' not in line and len(line) > 0:
                if int(line.split()[4].replace('%', '')) > 80:
                    return False
        return True
    except:
        return None
...
def verify_bgp_spine_prefixes(device):
    ...
    try:
        ...
        alog("verify_bgp_spine_prefixes: Data collection failed - %s" % repr(e))
        return False
...
def main():
    ...
    alog("Pre-Upgrade Health Check v1.0")
    ...
    if failures and not SCRIPT_DEBUG:
        assert False
